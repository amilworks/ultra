...
Fixtures generate tiny synthetic ``.h5`` files in-test (no committed
...
def test_is_hdf5_data_file_extension_gate():
    ...
    # A .dream3d file is a plain HDF5 container; the generic data gate must open it.
...
def test_untrusted_links_are_never_dereferenced(tmp_path):
    ...
    source = str(tmp_path / "linked.h5")
    with h5py.File(outside, "w") as f:
        f.create_dataset("secret", data=np.array([b"OUTSIDE-SECRET"], dtype="S32"))
        f.create_dataset("Labels", data=np.ones((2, 2, 2, 1), dtype="i4"))

    with h5py.File(source, "w") as f:
        f.attrs["FileVersion"] = "7.0"
        image = f.create_group("Containers").create_group("Image")
        ...
        cell["SoftLabels"] = h5py.SoftLink(intensity.name)
        cell["Labels"] = h5py.ExternalLink(outside, "/Labels")
        cell["ExternalSecret"] = h5py.ExternalLink(outside, "/secret")
        f["SoftSecret"] = h5py.SoftLink("/Containers/Image/CellData/ExternalSecret")
        f["ExternalSecret"] = h5py.ExternalLink(outside, "/secret")

    viewer = hdf5.build_hdf5_viewer_info(source)
    assert "OUTSIDE-SECRET" not in json.dumps(viewer)
    ...
    assert "/Containers/Image/CellData/Labels" not in paths  # external link

    for linked_path in (
        "/ExternalSecret",
        "/SoftSecret",
        "/Containers/Image/CellData/ExternalSecret",
        "/Containers/Image/CellData/Labels",
    ):
        with pytest.raises(hdf5.Hdf5DatasetNotFound):
    ...
    resolved = hdf5.dataset_summary(source, "/Containers/Image/CellData/SoftLabels")
