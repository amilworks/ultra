...
function useTextStream({
  ...
  const updateSegments = useCallback((text: string) => {
    if (modeRef.current === "fade") {
      try {
        ...
        if (!Segmenter) {
          throw new Error("Intl.Segmenter is unavailable")
        }
        const segmenter = new Segmenter(navigator.language, {
          granularity: "word",
        })
  ...
  const commitDisplayedText = useCallback(
    (nextText: string) => {
  ...
  const getAsyncChunkSize = useCallback(
    (pendingChars: number) => {
      ...
      return Math.min(
        softMax,
        Math.max(baseChunkSize, Math.ceil(pendingChars / 24))
      )
    },
    [getChunkSize]
  )
  ...
  const processStringTypewriter = useCallback(
    (text: string) => {
      ...
      const streamContent = (timestamp: number) => {
        ...
        commitDisplayedText(newDisplayedText)
    ...
    [commitDisplayedText, getProcessingDelay, getChunkSize, markComplete]
  )
  ...
  const processAsyncIterable = useCallback(
    async (stream: AsyncIterable<string>) => {
      ...
      const flushToTarget = async () => {
        while (
          !controller.signal.aborted &&
          displayedTextRef.current.length < targetText.length
        ) {
          const pendingChars = targetText.length - displayedTextRef.current.length
          const nextText = targetText.slice(
            0,
            displayedTextRef.current.length + getAsyncChunkSize(pendingChars)
          )
