...
const SIDEBAR_WIDTH_MOBILE = "min(88vw, 22.5rem)"
const SIDEBAR_WIDTH_MD = "13rem"
...
function Sidebar({
  ...
  const handleMobileSheetClickCapture = React.useCallback(
    (event: React.MouseEvent<HTMLDivElement>) => {
      ...
      if (event.target.closest('[data-sidebar-close-mobile="false"]')) {
        return
      }
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-mobile="true"
          className={cn(
            "bg-sidebar text-sidebar-foreground h-dvh w-(--sidebar-width) max-w-none overflow-hidden p-0 [&>button]:hidden",
            className
          )}
          style={
            {
        ...
        >
          ...
          <div
            ...
            onClickCapture={handleMobileSheetClickCapture}
          >
            {children}
          </div>
        </SheetContent>
...
function SidebarMenuButton({
  ...
  const { isMobileView, state } = useSidebar()
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent
        side="right"
        align="center"
        hidden={state !== "collapsed" || isMobileView}
        {...tooltip}
      />
    </Tooltip>
  )
...
// Spreadable prop bags for the mobile sheet's tap-to-close behavior (consumed by
// the data-sidebar-close-mobile click handling above).
