...
describe("composer intelligence selector", () => {
  it("keeps one mode control: the chip that leads the brief", () => {
    ...
    expect(composer).toMatch(/data-testid="composer-mode-chip"/);
    ...
    expect(styles).toMatch(/\.composer-chip-mode/);
  ...
  it("introduces Pro with the reusable one-time release notice, anchored to the chip", () => {
    expect(composer).toContain('import { OneTimeNotice } from "@/components/ui/one-time-notice";');
    expect(composer).toMatch(/<OneTimeNotice[\s\S]{0,700}\{modeChip\}[\s\S]{0,40}<\/OneTimeNotice>/);
