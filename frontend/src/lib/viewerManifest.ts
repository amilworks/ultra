 ...
 * Prefer Z (a focal/depth stack); fall back to T for a Z-less time-series — e.g. a
 * 61-frame movie whose axes are (t, c, y, x) — so those scrub their frames instead
 * of a single clamped plane. Returns count 1 (non-scrubbable) when neither axis has
 * depth, so a flat image keeps its static thumbnail.
 */
