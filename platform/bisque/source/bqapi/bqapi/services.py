...
class DatasetProxy (BaseServiceProxy):
    ...
    def _dataset_document (self, dataset_uniq, view='deep'):
        data = self.session.service('data_service')
        dataset = data.fetch (dataset_uniq, params={'view':view}, render='etree')
        if getattr(dataset, 'tag', None) != 'dataset' and len(dataset):
            nested = dataset.find('.//dataset')
            if nested is not None:
                dataset = nested
        return data, dataset

    def _dataset_member_uri (self, data, resource_uniq):
        return data.construct (resource_uniq)

    def _dataset_member_nodes (self, dataset, member_uri):
        return [
            value
            for value in dataset.findall('./value')
            if str(value.text or '').strip() == member_uri
        ]

    def _reindex_dataset_members (self, dataset):
        for indx, value in enumerate(dataset.findall('./value')):
            value.set('index', str(indx))
        return dataset
    ...
    def append_member (self, dataset_uniq, resource_uniq, **kw):
        """Append an element
        """
        data, dataset = self._dataset_document (dataset_uniq, view='deep')
        member_uri = self._dataset_member_uri (data, resource_uniq)
        members = self._dataset_member_nodes (dataset, member_uri)
        if len (members):
            return dataset
        member = etree.SubElement(dataset, 'value', type='object')
        member.text = member_uri
        self._reindex_dataset_members (dataset)
...
class ModuleProxy (BaseServiceProxy):
    ...
    def unregister (self, engine_url=None, module_uri=None, resource_uniq=None):
        params = {}
        if resource_uniq:
            params['resource_uniq'] = resource_uniq
        else:
            params['engine_uri'] = engine_url
            if module_uri:
                params['module_uri'] = module_uri
        return self.request (path='unregister_engine', params = params)
