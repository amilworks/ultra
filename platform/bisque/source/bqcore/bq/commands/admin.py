...
class server(object):
    ...
    def __init__(self, version):
        ...
        parser.add_option(
            ...
            action="store_true",
            default=False,
            help="try force start or stop: ignore old pid files etc.",
        )
...
class deploy(object):
    ...
    def deploy_public(self):
            ...
            try:
                ...
                for d, r in staticdirs:
                    if not os.path.isdir(r):
                        # Editable installs can resolve pkg_resources paths to
                        # namespace stubs in site-packages; fall back to the
                        # service source tree's sibling `public` directory.
                        module_file = getattr(service, "__file__", None)
                        if module_file:
                            candidate_public = os.path.abspath(
                                os.path.join(os.path.dirname(module_file), "..", "public")
                            )
                            if os.path.isdir(candidate_public):
                                r = candidate_public
                            else:
                                print(
                                    "Skipping static dir for %s: missing %s"
                                    % (x.name, r)
                                )
                                continue
                        else:
        ...
        # removed from dev dirs temporarily
        remove_safe("bqcore/bq/core/public/css/all_css.css")
        remove_safe("bqcore/bq/core/public/js/all_js.js")
        ...
        if self.args[0].startswith("init"):
            ...
            if len(x):
                if self.options.force:
                    ...
                    print(
                        (
                            "NO ACTION: System object initialized at %s "
                            % etree.tostring(x[0], encoding="unicode")
                        )
                    )
                    sys.exit(1)
