...
#
##############################################################################
# !!! XFIle is not currently being used, as it was too much error-prone, new approach is modified locks.py with portalocker
import errno
from os import name
import io
...
#             # Avoid unlocking zero or negative ranges
#             num_Start = 0
#             num_End = 2**63 - 1  # POSIX-safe "entire file"
...
#         """Locks the file.  num_LockFlags is a XOR'ed combination of the flags you want for this lock operation; num_Start is the start of the range of bytes you want to lock, and num_End is the end."""
#         log.info(f"[XFile] Attempting lock: flags={num_LockFlags}, start={num_Start}, end={num_End} file={self.file.name}")

#         # Check that the file is lockable; ie, not closed or already locked.
#         if self.is_locked() == True:
#             log.error("cannot lock file; file is already locked")
#             raise LockError("cannot lock file; file is already locked")
#         if self.closed == True:
#             log.error("cannot lock file; file is closed")
#             raise LockError("cannot lock file; file is closed")

#         # Check that the flags are an integer.
#         if type(num_LockFlags) not in [int, int]:
#             log.info("expected int or long for lock flags, got %s" % type(num_LockFlags).__name__)
#             raise LockError("expected int or long for lock flags, got %s" % type(num_LockFlags).__name__)

#         # Check that we're not doing LOCK_EX when in read mode on POSIX.
#         if name == "POSIX" and num_LockFlags | LOCK_EX == num_LockFlags and "r" in self.mode and "+" not in self.mode: # pylint: disable=unsupported-membership-test
#             log.error("LOCK_EX cannot be used in read mode on POSIX; use LOCK_SH instead")
#             raise LockError("LOCK_EX cannot be used in read mode on POSIX; use LOCK_SH instead")

#         # Check that num_Start and num_End are integers.
#         if type(num_Start) not in [int, int]:
#             log.error("expected int or long for lock range start, got %s" % type(num_Start).__name__)
#             raise LockError("expected int or long for lock range start, got %s" % type(num_Start).__name__)
#         if type(num_End) not in [int, int]:
#             log.error("expected int or long for lock range end, got %s" % type(num_End).__name__)
#             raise LockError("expected int or long for lock range end, got %s" % type(num_End).__name__)

#         # Check that the ranges are positive.
#         if num_Start < 0:
#             log.error("lock range start must be a positive integer")
#             raise LockError("lock range start must be a positive integer")
#         if num_End < 0:
#             log.error("lock range end must be a positive integer")
#             raise LockError("lock range end must be a positive integer")

#         # Check that the start and end are not the same.  The reason we ignore
#         # a 0 for both values is that 0 is the default.
#         if num_Start == num_End and num_Start != 0 and num_End != 0:
#             log.error("lock range start and lock range end must be different")
#             raise LockError("lock range start and lock range end must be different")
...
#     def unlock(self):
#         """Unlocks the file."""
#         log.info(f"[XFile] Attempting unlock: file={self.file.name} range={self.__num_Start}-{self.__num_End}")
#         if not self.is_locked():
#             log.error("cannot unlock file; file is already unlocked")
#             raise LockError("cannot unlock file; file is already unlocked")
#         if self.closed:
#             log.error("cannot unlock file; file is closed")
#             raise LockError("cannot unlock file; file is closed")

#         # Sanity check for unlock range
#         if not hasattr(self, "_XFile__num_Start") or not hasattr(self, "_XFile__num_End"):
#             log.error("unlock range not set; lock() may not have been called")
#             raise LockError("unlock range not set; lock() may not have been called")

#         if self.__num_End <= self.__num_Start:
#             # Avoid unlocking a zero-byte range, which may hang on POSIX
#             self.__num_Start = 0
#             self.__num_End = 2**63 - 1  # Large number to represent "entire file"
#         log.info("---- Unlocking %s, %s" % (self.__num_Start, self.__num_End))
#         # Unlock
#         self.__UnlockFile(self.__num_Start, self.__num_End)
