...
def convert_image2numpy(image_path):
    """
        ...
        First attempts to read the image with tifffile. If
    ...
    """
    try:
        # Use tifffile to read TIFF images (replaces deprecated libtiff)
        image = tifffile.imread(image_path)
        
        # Handle single vs multi-page TIFF files
        if image.ndim == 4:  # Multi-page TIFF (pages, height, width, channels)
            # Stack pages along the channel dimension
            if image.shape[3] == 1:  # Grayscale pages
                image = np.squeeze(image, axis=3)  # Remove single channel dimension
                if image.shape[0] > 1:  # Multiple pages
                    image = np.dstack([image[i] for i in range(image.shape[0])])
                else:
                    image = image[0]
            else:  # RGB pages - take first page
                image = image[0]
        elif image.ndim == 3:
            # Could be (height, width, channels) or (pages, height, width)
            if image.shape[2] <= 4:  # Likely channels (RGB, RGBA)
                pass  # Keep as is
            else:  # Likely pages for grayscale
                if image.shape[2] > 1:
                    image = np.dstack([image[:, :, i] for i in range(image.shape[2])])
                else:
                    image = image[:, :, 0]
        ...
        log.debug("Not a tiff file or tifffile failed!")
...
# !!! old codes, kept for reference, not used anymore
# import os
# import numpy as np
# import re
# import logging
