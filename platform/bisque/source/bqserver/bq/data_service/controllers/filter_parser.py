...
log = logging.getLogger("bq.data_service.filter_parser")


class FilterParseError(ValueError):
    """Raised when extract filter parser fails."""
...
def t_error(t):
    msg = "Illegal character '%s' in extract filter" % t.value[0]
    log.warning(msg)
...
def p_error(p):
    ...
    if token is None:
        msg = "Syntax error at end of extract filter"
    else:
        msg = "Syntax error near token '%s' in extract filter" % token
    log.warning(msg)
    raise FilterParseError(msg)
...
def filter_parse (text):
    ...
    parsed = parser.parse (text, lexer = lexer)
    if parsed is None:
        raise FilterParseError("Malformed extract filter")
    return parsed
