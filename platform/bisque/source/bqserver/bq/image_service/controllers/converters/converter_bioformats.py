...
#         cls.get_formats()
    ...
    def meta(cls, token, **kw):
        ...
        acquisition_date = misc.xpathtextnode(mee, '%s/ome:AcquisitionDate'%imagenodepath, namespaces=namespaces)
        rd['date_time'] = acquisition_date.replace('T', ' ') if isinstance(acquisition_date, str) else ''

        pixels_nodes = mee.xpath('%s/ome:Pixels'%imagenodepath, namespaces=namespaces)
        if len(pixels_nodes) < 1:
            return {}
    ...
    def convert(cls, token, ofnm, fmt=None, extra=None, **kw):
        ...
        series_int = misc.safeint(series, 0)
        if series_int >= 0:
        ...
        if tmp is None:
            ...
            if r is None or not os.path.exists(tmp):
                return None
            os.rename(tmp, ofnm)
            return ofnm
...
try:
    ...
    log.warning("Bioformats unavailable")
