...
#         cls.get_formats()
    ...
    def writeHistogram(cls, token, ofnm, **kw):
        ...
        with open(ofnm, 'wb') as f:
            f.write(struct.pack('<cccc', b'B', b'I', b'M', b'1')) # header
            f.write(struct.pack('<cccc', b'I', b'H', b'S', b'1')) # spec
            f.write(struct.pack('<L', channels)) # number of histograms
            # write histograms
            for c in range(channels):
                f.write(struct.pack('<cccc', b'B', b'I', b'M', b'1')) # header
                f.write(struct.pack('<cccc', b'H', b'S', b'T', b'1')) # spec
...
try:
    ...
    log.warning("Openslide Unavailable")
