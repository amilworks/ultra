...
converters_preferred_order = ["openslide", "imgcnv", "pillow", "ImarisConvert", "bioformats", "ffmpeg"]
...
from .converters.converter_pillow import ConverterPillow
...
class ImageServer(object):
    ...
    converters = ConverterDict(
        [
            ...
            (ConverterPillow.name, ConverterPillow()),
    ...
    def init_converters(cls):
        ...
        if ConverterImgcnv.name not in cls.converters:
            log.warning(
                "imgcnv was not found, it is required for most of image service operations! Make sure to install it!"
            )

        cls.writable_formats = cls.converters.converters(
            readable=False, writable=True, multipage=False
        )
    ...
    def __init__(self, work_dir, run_dir):
        ...
        self.operations = PluginManager(
            "operation",
            os.path.join(self.basepath, "operations"),
            BaseOperation,
            server=self,
        )
    ...
    def process_queue(self, token):
        ...
        if token.isFile() and len(token.queue) > 0 and not os.path.exists(ofile):
            ...
            log.info(
                "Executing enqueued commands %s: %s to %s with %s",
                token.resource_id,
                ifile,
                ofile,
                command,
    ...
    def imageconvert(self, token, ifnm, ofnm, fmt=None, extra=None, dims=None, **kw):
        ...
        if (
            dims.get("image_num_x", 0) > 4000
            and dims.get("image_num_y", 0) > 4000
            and "tiff" in fmt
            and "-options" not in command
        ):
            command.extend(
                [
                    "-options",
        ...
        imgcnv = self.converters.get(ConverterImgcnv.name)
        if kw.get("try_imgcnv", True) is True and imgcnv is not None:
            r = imgcnv.convert(
                token, ofnm, fmt=fmt, extra=command, respect_command_inputs=True, **kw
            )
            if r is not None:
                return r
        ...
        # Prefer imgcnv for the final conversion step when available.
        if imgcnv is not None:
            return imgcnv.convert(
                ProcessToken(ifnm=ometiff), ofnm, fmt=fmt, extra=command
            )

        # Development fallback (for environments without imgcnv).
        preferred = []
        if dims.get("converter") in self.converters:
            preferred.append(dims.get("converter"))
        preferred.extend([n for n in self.converters.keys() if n not in preferred])
        for n in preferred:
            c = self.converters[n]
            if callable(getattr(c, "convert", None)) is not True:
                continue
            r = c.convert(ProcessToken(ifnm=ometiff), ofnm, fmt=fmt, extra=command, **kw)
            if r is not None:
                return r

        raise ImageServiceException(
            responses.UNSUPPORTED_MEDIA_TYPE,
            "Convert failed: no suitable converter available",
        )
    ...
    def initialWorkPath(self, image_id, user_name, series=0):
        ...
        safe_user = ""
        if user_name:
            normalized_user = str(user_name).replace("\\", "/")
            user_parts = [p for p in normalized_user.split("/") if p not in ("", ".", "..")]
            if len(user_parts) > 0:
                safe_user = os.path.join(*user_parts)

        return os.path.realpath(os.path.join(self.workdir, safe_user, subdir, image_id))
    ...
    def request(self, method, token, arguments):
        ...
        if method not in self.operations.plugins:
            raise ImageServiceException(
                responses.BAD_REQUEST, "Requested operation does not exist: %s" % method
            )
    ...
    def process(self, url, ident, resource=None, **kw):
        ...
        if ident is not None:
            ...
            if len(query) > 0:
                series = subpath or 0
                workpath = self.initialWorkPath(
                    ident, user_name=kw.get("user_name", None), series=series
                )
                token.setFile(workpath, series=series)
                # log.info(f"---token is {token} workpath is {workpath}")
                token.dims = self.getImageInfo(
                    filename=token.data,
                    series=token.series,
                    infofile="%s.info" % token.data,
                    meta=kw.get("imagemeta", None),
                )
                ...
                if token.dims is None:
                    ...
                    token.init(
                        resource_id=ident,
                        ifnm=token.data,
                        imagemeta=kw.get("imagemeta", None),
                    ...
                    for action, args in query:
                        try:
                            ...
                            if callable(getattr(service, "dryrun", None)):
                                log.debug(
                                    "DRY run: %s calling dryrun: %s",
                                    action,
                                    token.dims["image_num_t"],
                                )
                                token = service.dryrun(token, args)
                            else:
                                log.debug(
                                    "DRY run: %s calling action: %s",
                                    action,
                                    token.dims["image_num_t"],
                                )
                    ...
                    if dryrun_valid and token.isFile() and os.path.exists(localpath):
                        with Locks(token.data, failonread=(not block_reads)) as l:
                            if (
                                ...
                                # !!! removing the following line, it was seems to be added to handle issue, but now it is causing problems and seems to be not needed
            ...
            workpath = self.ensureWorkPath(
                b.path, ident, user_name=kw.get("user_name", None), series=series
            )
            token.setFile(workpath, series=series)
            token.init(
                resource_id=ident,
            ...
            if len(query) > 0:
                token.dims = self.getImageInfo(
                    filename=token.first_input_file(),
                    series=token.series,
                    infofile="%s.info" % token.data,
                    meta=token.meta,
                )
        ...
        if (
            token.isFile()
            and not token.isImage()
            and not token.isText()
            and not token.hasFileName()
        ):
...
try:
    ...
    log.warning("ImageServer is not available")
