...
class TileOperation(BaseOperation):
    ...
    def action(self, token, arg):
        ...
        # Development fallback for non-pyramidal decoders (for example Pillow).
        # This keeps tile requests working when imgcnv is unavailable.
        fallback_ofname = '%s.tif' % ofname
        for n, c in self.server.converters.items():
            if n == ConverterImgcnv.name:
                continue
            if callable(getattr(c, "tile", None)) is not True:
                continue
            r = c.tile(token, fallback_ofname, level, tnx, tny, tsz)
            if r is not None and os.path.exists(fallback_ofname):
                return token.setImage(fallback_ofname, fmt=default_format, dims=info, input=fallback_ofname)
