...
def upgrade():
    ...
    root = urllib.parse.urlsplit(bisque_root)
    ...
    class Taggable(Base):
        ...
        def __str__(self):
            return "<%s id=%s name=%s value=%s ts=%s document=%s>" % (self.resource_type, self.id, self.resource_name, self.resource_value, self.ts, self.document_id)
    ...
    for tag in alltags:
        ...
        if target:
            ...
            newurl  = urllib.parse.urlunsplit ( parts )
