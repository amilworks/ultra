...
class PythonScriptWrapper(object):
    ...
    def upload_results(self, bq):
        ...
        for resource in (nonimage_tag.findall(".//*[@type]") + outputs_tag.findall("./*[@type='image']")): 
            ...
            output_uri = self._normalize_resource_uri_for_external_clients(output_etree_Element.get('value'))
            if output_uri:
                output_etree_Element.set('value', output_uri)
            # log.info(f"***** Uploaded output {resource_type} '{resource_name}' to {output_etree_Element.get('value')}")
            log.info("***** Uploaded output %s '%s' to %s" % (resource_type, resource_name, output_uri))

            # Set the value attribute of the each resource's tag to its corresponding resource uri
            resource.set('value', output_uri or '')
            ...
            if resource in outputs_tag.findall("./*[@type='image']"):
                ...
                non_image_value[resource_name] = output_uri
        ...
        # SAMPLE LOG
        # ['<tag name="OutImage" type="image" value="http://example.invalid:8080/data_service/00-ExhzBeQiaX5F858qNjqXzM">\n               <template>\n                    <tag name="label" value="Edge Image" />\n               </template>\n          </tag>\n     ']
...
#        print(inputs_tag)
        for input_resource in inputs_tag.findall("./*[@type='resource']"):
            ...
            # Normalize loopback URLs before loading resource metadata from inside Docker.
            input_uri = self._normalize_resource_uri_for_container(getattr(self.options, input_name))
            resource_obj = bq.load(input_uri)
            """
            bq.load returns bqapi.bqclass.BQImage object or bqapi.bqclass.BQResource object. Ex:
            resource_obj: (image:name=whale.jpeg,value=file://admin/2022-02-25/whale.jpeg,type=None,uri=http://example.invalid:8080/data_service/00-pkGCYS4SPCtQVcdZUUj4sX,ts=2022-02-25T17:05:13.289578,resource_uniq=00-pkGCYS4SPCtQVcdZUUj4sX)

            resource_obj: (resource:name=yolov5s.pt,type=None,uri=http://example.invalid:8080/data_service/00-D9e6xVPhU93JtZjZZtwkLm,ts=2022-02-26T01:08:26.198330,resource_uniq=00-D9e6xVPhU93JtZjZZtwkLm) (PythonScriptWrapper.py:137)

            resource_obj: (resource:name=test.npy,type=None,uri=http://example.invalid:8080/data_service/00-EC53Rcbj8do86aXpea2cgW,ts=2022-02-26T01:17:12.312780,resource_uniq=00-EC53Rcbj8do86aXpea2cgW) (PythonScriptWrapper.py:137)
            """
            ...
            resource_uri = self._normalize_resource_uri_for_container(resource_obj.uri)
            fetch_blob_output = fetch_blob(bq, resource_uri, dest=input_path_dict[input_name])
    ...
    @staticmethod
    def _normalize_resource_uri_for_container(uri):
        """Map loopback BisQue URLs to host.docker.internal when running inside Docker."""
        if not uri or not os.path.exists('/.dockerenv'):
            return uri
        return (uri
    ...
    @staticmethod
    def _normalize_resource_uri_for_external_clients(uri):
        """Emit host-agnostic output paths so UI/API works on localhost or 127.0.0.1."""
        if not uri:
            return uri
        parsed = urlsplit(uri)
