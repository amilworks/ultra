...
class DatasetProxy (BaseServiceProxy):
    ...
    def append_member (self, dataset_uniq, resource_uniq, **kw):
        ...
        """
        data = self.session.service('data_service')
        dataset = data.fetch ( dataset_uniq,  params = {'view':'deep'}, render='etree')
        member_uri = data.construct (resource_uniq)
        members = dataset.xpath ('value[text()="%s"]' % member_uri)
        if len (members):
            return dataset
        member = etree.SubElement(dataset, 'value', type='object')
        member.text = member_uri
        for indx, val in enumerate(dataset.iter('value')):
            val.set('index', str(indx))
