...
public class BQImage extends BQObject{
	...
	public BQImage(Integer x_, Integer y_, Integer z_, Integer t_, Integer ch_){
		x = x_;
		y = y_;
		z = z_;
		t = t_;
    ...
    public BQImagePixels getPixels(){
        return new BQImagePixels(src);
    }
    public void getPixels2(){
		...
		try{
			...
			for(int ti=0; ti<t; ti++){
				for(int zi=0; zi<z; zi++){
				    String urlQuery ="";
				    if (z > 1 && t > 1)   urlQuery = "slice=,," + (zi+1) + "," + (ti+1) +",";
				    else if (z == 1 && t > 1)  urlQuery = "slice=,,,"+ (ti+1) + "," ;
				    else if (z > 1 && t == 1)  urlQuery = "slice=,," + (zi+1) + ",,";
        ...
        try{
            ...
            for(int ti=0; ti<t; ti++){
                for(int zi=0; zi<z; zi++){
                    ...
                    if (z > 1 && t > 1)     urlQuery = "?slice=,," + new Integer(zi+1).toString() + "," + new Integer(ti+1).toString() + "&remap="+selectedChannel+"&format=raw";
                    if (z == 1 && t > 1)    urlQuery = "?slice=,,,"+ new Integer(ti+1).toString() + "&remap="+selectedChannel+ "&format=raw";
                    if (z > 1 && t == 1)    urlQuery = "?slice=,," + new Integer(zi+1).toString() + ","+ "&remap="+selectedChannel+ "&format=raw";
	...
	public void deleteData(String imageUri){
		try{
			String response = null;
			String boundary = "--------------------" + Long.toString(System.currentTimeMillis(), 30);

			URL server = new URL(imageUri); 
