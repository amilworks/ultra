...
require_clean_tree() {
  ...
  if ! git diff --quiet --ignore-submodules -- || ! git diff --cached --quiet --ignore-submodules --; then
    echo "Refusing to build an immutable release from a dirty tracked worktree." >&2
    echo "Commit or stash tracked changes first, or set ALLOW_DIRTY_RELEASE=1 for local dry-runs." >&2
    exit 1
  fi
}
...
export RELEASE_SHA RELEASE_NAME GOOS_VALUE GOARCH_VALUE CGO_ENABLED_VALUE MANIFEST_IN_RELEASE MANIFEST_OUT
python3 <<'PY'
import datetime
import json
import os
