...
start_control() {
  ...
  start_detached "$CONTROL_PID_FILE" "$LOG_DIR/control.log" "$ROOT/backend/controlplane" "ultra-main-control" \
    ...
    ULTRA_CONTROL_IMAGE_SERVICE_URL="$ULTRA_CONTROL_IMAGE_SERVICE_URL" \
    ULTRA_CONTROL_VLM_ENABLED="${ULTRA_CONTROL_VLM_ENABLED:-}" \
    ULTRA_CONTROL_VLM_BASE_URL="${ULTRA_CONTROL_VLM_BASE_URL:-}" \
    ULTRA_CONTROL_VLM_API_KEY_FILE="${ULTRA_CONTROL_VLM_API_KEY_FILE:-}" \
    ULTRA_CONTROL_VLM_MODEL="${ULTRA_CONTROL_VLM_MODEL:-}" \
...
start_deepagents_worker() {
  ...
  if [ "$python_bin" = "uv" ]; then
    start_detached "$WORKER_PID_FILE" "$LOG_DIR/deepagents-worker.log" "$ROOT/backend/deepagents_runtime" "ultra-main-worker" \
      ...
      ULTRA_DEEPAGENTS_MODEL_STREAM_IDLE_TIMEOUT_SECONDS="${ULTRA_DEEPAGENTS_MODEL_STREAM_IDLE_TIMEOUT_SECONDS:-3600}" \
      ULTRA_DEEPAGENTS_SUBAGENT_TASK_TIMEOUT_SECONDS="${ULTRA_DEEPAGENTS_SUBAGENT_TASK_TIMEOUT_SECONDS:-1800}" \
      ...
      ULTRA_DEEPAGENTS_BUILDER_WORKER_BASE_URL="${ULTRA_DEEPAGENTS_BUILDER_WORKER_BASE_URL:-}" \
      ULTRA_DEEPAGENTS_BUILDER_WORKER_MODEL="${ULTRA_DEEPAGENTS_BUILDER_WORKER_MODEL:-}" \
      ULTRA_DEEPAGENTS_BUILDER_WORKER_API_KEY_FILE="${ULTRA_DEEPAGENTS_BUILDER_WORKER_API_KEY_FILE:-}" \
      ULTRA_DEEPAGENTS_BUILDER_WORKER_MAX_INPUT_TOKENS="${ULTRA_DEEPAGENTS_BUILDER_WORKER_MAX_INPUT_TOKENS:-0}" \
