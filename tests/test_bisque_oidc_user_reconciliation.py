...
def test_auth_service_reconciles_missing_bq_user() -> None:
    auth_service = (
        ROOT
        / "platform/bisque/source/bqserver/bq/client_service/controllers/auth_service.py"
    ).read_text()
...
def test_bootstrap_recovers_missing_admin_bq_user() -> None:
    bootstrap = (
        ROOT
        / "platform/bisque/source/bqcore/bq/websetup/bootstrap.py"
    ).read_text()
    assert "Bootstrap found TG admin without BQUser; creating missing BQUser record" in bootstrap
    assert "admin_tg = model.DBSession.query(model.User).filter_by(user_name='admin').first()" in bootstrap


def test_admin_ui_blocks_local_user_creation_in_oidc_mode() -> None:
    admin_service = (
        ROOT
        / "platform/bisque/source/bqserver/bq/admin_service/controllers/service.py"
    ).read_text()
    manager_js = (
        ROOT
        / "platform/bisque/source/bqcore/bq/core/public/js/admin/BQ.user.Manager.js"
    ).read_text()
