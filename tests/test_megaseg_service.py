...
def test_megaseg_service_materializes_public_remote_tiff_sources(monkeypatch, tmp_path):
    checkpoint = tmp_path / "epoch_650.ckpt"
    checkpoint.write_text("checkpoint", encoding="utf-8")
    settings = megaseg_service_app.ServiceSettings(
        checkpoint_path=checkpoint,
    ...
    class _FakeFs:
        ...
        def exists(self, path: str) -> bool:
            return path == "allencell/aics/example.ome.tiff"

        def open(self, path: str, mode: str = "rb"):
            assert path == "allencell/aics/example.ome.tiff"
            assert "r" in mode
            return BytesIO(b"remote-tiff-bytes")

    def fake_url_to_fs(uri: str, **kwargs):
        assert uri == "s3://allencell/aics/example.ome.tiff"
        if not kwargs:
            raise RuntimeError("Unable to locate credentials")
        assert kwargs == {"anon": True}
        return _FakeFs(), "allencell/aics/example.ome.tiff"

    monkeypatch.setattr(megaseg_service_app, "run_megaseg_batch", fake_run_megaseg_batch)
    monkeypatch.setattr(megaseg_service_app, "url_to_fs", fake_url_to_fs, raising=False)
    ...
    with TestClient(app) as client:
        response = client.post(
            ...
            json={
                ...
                "save_visualizations": False,
                "generate_report": False,
                "sources": [{"uri": "s3://allencell/aics/example.ome.tiff"}],
            },
        )
        assert response.status_code == 202
        job_id = response.json()["job_id"]
    ...
    assert captured_sources
    materialized_path = Path(captured_sources[0])
    assert materialized_path.is_file()
    assert materialized_path.read_bytes() == b"remote-tiff-bytes"
    assert str(materialized_path).startswith(str(settings.artifact_root / job_id / "inputs"))
